  ...
  return (
    <div className="bg-surface border border-border rounded-lg overflow-hidden hover:border-secondary/50 transition-all hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <div className="p-4 flex flex-col gap-2">
        <h3 className="text-base font-semibold text-primary line-clamp-2">{item.title}</h3>
        {item.subtitle && (
          <p className="text-sm text-secondary line-clamp-2 flex-1">{item.subtitle}</p>
        )}
        {item.source_url && (
          <a href={item.source_url} target="_blank" rel="noopener noreferrer"
            className="text-xs text-accent hover:text-accent/80 flex items-center gap-1 w-fit mt-1">
