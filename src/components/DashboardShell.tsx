...
import {
  Zap, Target, Inbox, HeartPulse, Newspaper, Sparkles,
...
type Surface = 'now' | 'pipeline' | 'intake' | 'news';
...
export default function DashboardShell({ initialItems, initialStats, initialApplications }: DashboardShellProps) {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (detail === 'now' || detail === 'pipeline' || detail === 'intake' || detail === 'news') {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (goPending) {
        ...
        if (k === 'e') { setActiveSurface('news');   cancelGo(); e.preventDefault(); return; }
  ...
  const paletteCommands: PaletteCommand[] = useMemo(() => {
    const cmds: PaletteCommand[] = [
      ...
      { id: 'go-news',     group: 'Navigate', label: 'Go to News',     hint: 'g e', icon: PaletteIcons.Now,      run: () => setActiveSurface('news') },
