...
import { ExternalLink, Briefcase, MapPin, DollarSign, Zap } from 'lucide-react';
...
/**
 * Job pipeline card with company, role, location, salary, score, and easy apply badge
 * Includes priority-based left border and hover lift effect
 */
export default function JobCard({ item }: JobCardProps) {
  ...
  // Determine score-based left border color
  const getScoreBorderColor = () => {
    if (!item.score) return 'border-secondary';
    if (item.score >= 65) return 'border-success';
    if (item.score >= 50) return 'border-warning';
    return 'border-secondary';
  };

  return (
    <div className={`bg-surface border border-border border-l-4 ${getScoreBorderColor()} rounded-lg p-4 hover:border-secondary/50 transition-all hover:-translate-y-0.5 hover:shadow-lg`}>
