...
interface ActionCardProps {
  /** The triage item with an action to approve/reject */
  item: TriageItem;
...
const actionMeta: Record<NonNullable<ActionType>, { label: string; icon: typeof Send; color: string }> = {
  ...
  accept_connection: { label: 'Accept Connection', icon: UserPlus, color: 'text-success' },
  review_document: { label: 'Review Document', icon: FileText, color: 'text-warning' },
  reply_email: { label: 'Reply Email', icon: Mail, color: 'text-info' },
};
...
/**
 * A2UI Action Card — the core agentic UI component
 * Renders a pre-prepared action with approve/reject controls
 * Follows AG-UI Delegation Pattern: users monitor plans and intervene safely
 */
export default function ActionCard({ item, onApprove, onReject }: ActionCardProps) {
  ...
  /** Build a source URL from the item */
  ...
  return (
    ...
    >
      ...
      {isPending && (
        <div className="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-warning via-warning/60 to-transparent" />
      )}

      <div className="p-4 pl-5">
        {/* Header row: source, action type, score */}
        ...
        {/* Title and subtitle — linked when source available */}
        {sourceLink ? (
          <a
            href={sourceLink}
            target="_blank"
            rel="noopener noreferrer"
            className="block text-base font-semibold text-primary mb-1 leading-tight hover:text-accent transition-colors"
          >
            {item.title}
          </a>
        ) : (
        ...
        {/* Cover letter preview (for job actions) */}
        {item.cover_letter && (
          <div className="mb-3">
            <button
            ...
            >
              <FileText size={14} />
              {expanded ? 'Hide' : 'Preview'} Cover Letter
              {expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            </button>
            {expanded && (
              <div className="mt-2 p-3 bg-elevated/80 rounded border border-border/50 text-xs text-secondary/90 leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto font-sans">
                {item.cover_letter}
              </div>
            )}
          </div>
        ...
        {/* Draft reply preview (for message/email actions) */}
        {item.draft_reply && !item.cover_letter && (
          <div className="mb-3">
            ...
            >
              <Mail size={14} />
              {expanded ? 'Hide' : 'Preview'} Draft Reply
              {expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            </button>
            {expanded && (
        ...
        {isPending && (
          <div className="flex items-center gap-2 mt-4 pt-3 border-t border-border/50">
            ...
            <button
              ...
              disabled={isProcessing}
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg bg-danger/10 text-danger/80 hover:bg-danger/20 hover:text-danger border border-danger/15 hover:border-danger/30 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-danger"
            >
              <XCircle size={16} />
              Reject
            </button>
